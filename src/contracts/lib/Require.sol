...
library Require {
        ...
        returns (bytes memory)
    {
        ...
        for (uint256 i = 32; i > 0; ) {
            ...
            // find the last non-zero bytes1 in order to determine the length
            if (result[i] != 0) {
                uint256 length = i + 1;
        ...
        returns (bytes memory)
    {
        ...
        // for each bytes1 (starting from the lowest byte), populate the result with two characters
        for (uint256 i = 0; i < 20; i++) {
            // each bytes1 takes two characters
            uint256 shift = i * 2;
